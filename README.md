...
ニュース画像の転載は基本 NG のため、本プロトタイプでは **全てのサムネイル／アイキャッチを OKLCH グラデーションの抽象表現** で構築しています（`.thumb--blue` / `--amber` / `--violet` / `--teal` / `--rose` / `--lime`）。

実運用での画像方針:
1. **テキスト重視** ＋ 元記事へのリンクカード（最も安全・Bloomberg/FT 系と相性◎）
2. **Unsplash / Pexels**（商用 OK・帰属不要）の抽象写真
3. ローカル Stable Diffusion 等の **AI 生成画像**（無料・コスト懸念なし）

API 従量課金のかかる画像生成サービス（DALL·E / Imagen 等）は使用していません。
