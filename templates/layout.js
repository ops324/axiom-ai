...
export function header(dateLabel, activeNav = 'トップ', base = '') {
  const nav = ['トップ', '基盤モデル', '研究', '産業応用', '規制・倫理', 'スタートアップ', 'ハードウェア', 'オピニオン', 'データ'];
  const items = nav
    .map((n) => {
      const href = n === 'トップ' ? `${base}index.html` : '#';
      const cur = n === activeNav ? ' aria-current="page"' : '';
      return `          <li><a href="${href}"${cur}>${n}</a></li>`;
    })
    .join('\n');
  return `  <!-- ============== HEADER ============== -->
    ...
    <div class="container">
      <div class="site-header__top">
        ...
        <div class="site-header__meta">
          <span>東京</span>
          <time>${dateLabel}</time>
          <a href="#signin" class="btn btn--ghost">ログイン</a>
          <a href="#subscribe" class="btn btn--primary">購読 ¥0/月</a>
        </div>
      </div>
